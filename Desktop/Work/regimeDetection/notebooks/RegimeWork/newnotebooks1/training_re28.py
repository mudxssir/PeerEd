# -*- coding: utf-8 -*-
"""
Feature Engineering + Hierarchical Regime Detection – re28.

re28 is based on re27. Adds 5 EDA-backed units: dead feature expansion (slope/momentum/
safe_*/percentile_20/correlation groups), direction-specific hostile exclusions, rolling
window option + SHORT MIN_TRAIN=250d, VWAP deviation + IV rank features, and a runtime
assertion for feature exclusion completeness.
...
if 'direction' in df.columns:
    for _dir in sorted(df['direction'].dropna().unique()):
        _dir_file = OUTPUT_DIR / f"AAPL_tier4_features_no_target_re28_{_dir}.parquet"
...
TEST_TDAYS     = 28
EMBARGO_TDAYS  = 3
TRAIN_TDAYS_SHORT  = 250   # 250d reduces cold-start effect (-7.6% Fold1→2 at 200d)
USE_ROLLING_WINDOW = True  # rolling caps train window; prevents low-ATR 2023 data contaminating high-ATR 2025 folds
...
# _DEAD_FEATURES_RE28: EDA-confirmed noise/redundant features (re28 audit)
# Slope group: mean |r|=0.028 (long), 0.015 (short) — 6-10x weaker than rsi_20
# Momentum group: mean |r|=0.056 (long), 0.023 (short) — weak independent signal
# Safe_* variants: r=1.0 with base versions — perfect clones
# Percentile_20 variants: r=0.978 with zscore_20 — same monotone transform
# Unstable: safe_net_gex collapses 89% in recent periods; gamma_flip_pct decays 65%
_DEAD_FEATURES_RE28 = frozenset([
    # Slope group (14 features — mean |r|<0.028, noise floor)
    'call_wall_slope_5min', 'put_wall_slope_5min',
    'max_positive_net_gex_slope_5min', 'max_negative_net_gex_slope_5min',
    'slope_net_gex_6_5min', 'slope_net_gex_12_5min',
    'slope_net_gex_6_zscore_20_5min', 'slope_net_gex_12_zscore_20_5min',
    'slope_net_gex_6_percentile_20_5min', 'slope_net_gex_12_percentile_20_5min',
    'gex_ratio_slope_5min', 'net_notional_delta_momentum_slope_5min',
    'put_call_ratio_slope_5min', 'call_put_bias_slope_5min',
    # Momentum group (12 features — mean |r|<0.06, short mean |r|=0.023)
    'gex_ratio_momentum_5min', 'net_gex_momentum_5min', 'abs_gex_momentum_5min',
    'call_wall_momentum_5min', 'put_wall_momentum_5min',
    'max_positive_net_gex_momentum_5min', 'max_negative_net_gex_momentum_5min',
    'vma_net_gex_6_momentum_5min', 'vma_net_gex_12_momentum_5min',
    'put_call_ratio_momentum_5min', 'net_notional_delta_momentum_5min',
    'call_put_bias_momentum_5min',
    # Safe_* variants (r=1.0 with base — perfect clones)
    'safe_net_gex_5min',  # r=1.0 with net_gex_5min; also 89% signal collapse in P3
    # Percentile_20 variants (r=0.978 with zscore_20 — rsi_20 already covers this)
    'call_wall_percentile_20_5min', 'put_wall_percentile_20_5min',
    'max_positive_net_gex_percentile_20_5min', 'max_negative_net_gex_percentile_20_5min',
    'gex_ratio_percentile_20_5min', 'net_gex_percentile_20_5min',
    'abs_gex_percentile_20_5min', 'vma_net_gex_6_percentile_20_5min',
    'vma_net_gex_12_percentile_20_5min', 'net_notional_delta_percentile_20_5min',
    'put_call_ratio_percentile_20_5min', 'call_put_bias_percentile_20_5min',
    'gamma_flip_percentile_20_5min',  # also 65% temporal signal decay
    # Correlation group (mean |r|<0.05 — redundant with GEX levels)
    'net_gex_correlation_20_5min', 'abs_gex_correlation_20_5min',
    'call_wall_correlation_20_5min', 'put_wall_correlation_20_5min',
    'gex_ratio_correlation_20_5min', 'vma_net_gex_12_correlation_20_5min',
])
...
# Unit D: VWAP deviation and IV rank (only if source data available)
_vwap_cols = [c for c in df.columns if 'vwap' in c.lower() and 'lag1' not in c]
if _vwap_cols and 'close' in df.columns:
    _vwap_col = _vwap_cols[0]
    _atr_base = df_raw['atr_brick_size'].clip(lower=0.01) if 'atr_brick_size' in df_raw.columns else pd.Series(1.0, index=df.index)
    df['vwap_deviation_atr'] = (df['close'] - df[_vwap_col]).abs() / _atr_base.values

_iv_col = next((c for c in ['stock_iv_30d_5min', 'stock_iv_30d_5min_lag1', 'stock_iv_7d_5min']
                if c in df.columns), None)
if _iv_col is not None:
    _iv_max252 = df[_iv_col].rolling(252, min_periods=60).max()
    _iv_min252 = df[_iv_col].rolling(252, min_periods=60).min()
    _iv_range  = (_iv_max252 - _iv_min252).clip(lower=1e-6)
    df['iv_rank_252d'] = ((df[_iv_col] - _iv_min252) / _iv_range).clip(0.0, 1.0)
...
EXCLUDE_FROM_FEATURES = set(META_COLS + FORBIDDEN_COLUMNS) | _L1_EXCLUDE_FEATURES | _DEAD_FEATURES_RE28
...
# Unit E: sanity check — no target-derivative features should reach CANDIDATE_COLS
_leaked_l1 = [c for c in CANDIDATE_COLS if c in _L1_EXCLUDE_FEATURES]
if _leaked_l1:
    raise RuntimeError(f"re28: target-derivative features in CANDIDATE_COLS: {_leaked_l1}")

print(f"\n✓ State features : {len(STATE_FEATURES)}")
print(f"  Candidate features (trade-level pool) : {len(CANDIDATE_COLS)}")
print(f"  Dead features excluded (re17f): {len(_DEAD_FEATURES_RE17)} | (re28): {len(_DEAD_FEATURES_RE28)}")
...
_bad_cols.update((_DEAD_FEATURES_RE17 | _DEAD_FEATURES_RE28) & set(CANDIDATE_COLS))
...
def _compute_dedup_cols(df, candidate_cols, target_col='regime_target', corr_threshold=0.85):
    ...
    _valid = [c for c in candidate_cols
              if c in df.columns and c not in _DEAD_FEATURES_RE17 and c not in _DEAD_FEATURES_RE28]
    ...
    _kept.update(_c for _c in candidate_cols
                 if _c not in _G.nodes and _c not in _DEAD_FEATURES_RE17 and _c not in _DEAD_FEATURES_RE28)
...
# Unit B: direction-specific hostile feature exclusions (EDA: 15 sign-flip features confirmed)
# Long-hostile: near-zero/negative signal for long model
_HOSTILE_LONG = frozenset([
    'vma_net_gex_12_zscore_20_5min',    # r_long=+0.066, r_short=-0.142 (sign flip)
    'vma_net_gex_6_zscore_20_5min',     # r_long=+0.035, r_short=-0.123 (sign flip)
    'vma_net_gex_12_5min',              # r_long=+0.077, r_short=-0.077 (sign flip)
    'vma_net_gex_6_5min',               # r_long=+0.071, r_short=-0.077 (sign flip)
])
# Short-hostile: near-zero/negative signal for short model
_HOSTILE_SHORT = frozenset([
    'net_gex_5min',                     # r_long=+0.122, r_short=-0.075 (sign flip)
    'vma_net_gex_6_5min',               # r_long=+0.121, r_short=-0.077 (sign flip)
    'vma_net_gex_12_5min',              # r_long=+0.118, r_short=-0.077 (sign flip)
    'net_gex_accel_5min',               # r_long=+0.074, r_short=+0.002 (near-zero for short)
])
_CANDIDATE_COLS_LONG  = [c for c in _CANDIDATE_COLS_LONG  if c not in _HOSTILE_LONG]
_CANDIDATE_COLS_SHORT = [c for c in _CANDIDATE_COLS_SHORT if c not in _HOSTILE_SHORT]
...
print(f"  Long  candidate pool: {len(_CANDIDATE_COLS_LONG)} features "
      f"({len(_LONG_SPECIFIC_FEATS)} long-specific added, {len(_HOSTILE_LONG)} hostile excluded)")
print(f"  Short candidate pool: {len(_CANDIDATE_COLS_SHORT)} features "
      f"({len(_SHORT_SPECIFIC_FEATS)} short-specific added, {len(_HOSTILE_SHORT)} hostile excluded)")
...
def generate_wf_folds(df, train_tdays=TRAIN_TDAYS,
    ...
    while ptr + embargo_tdays + test_tdays <= n_dates:
        if rolling and max_train is not None:
            _effective_start = max(0, ptr - max_train)
            train_start = unique_dates[_effective_start]
        else:
...
def discover_states_for_core(train_core_df, core_state):
    ...
    sf_cols = [c for c in STATE_FEATURES
               if c in train_core_df.columns and c not in _DEAD_FEATURES_RE17 and c not in _DEAD_FEATURES_RE28]
    if len(sf_cols) < 3:
        return None
...
def _run_l2_bucket(l2_train_raw_bkt, l2_test_raw_bkt, bucket_name, fold_num,
    ...
    * _preds = top-30% indicator by predicted rank.
    ...
    _excl_cols = set(FORBIDDEN_COLUMNS) | _DEAD_FEATURES_RE17 | _DEAD_FEATURES_RE28 | {
...
for _TRAIN_DIR in ['long', 'short']:
    ...
    # Unit C: direction-specific fold construction (rolling window + SHORT MIN_TRAIN=250d)
    if _TRAIN_DIR == 'short':
        _dir_folds = generate_wf_folds(df, train_tdays=TRAIN_TDAYS_SHORT, test_tdays=TEST_TDAYS,
                                       embargo_tdays=EMBARGO_TDAYS, rolling=USE_ROLLING_WINDOW,
                                       max_train=TRAIN_TDAYS_SHORT)
    else:
        _dir_folds = generate_wf_folds(df, train_tdays=TRAIN_TDAYS, test_tdays=TEST_TDAYS,
                                       embargo_tdays=EMBARGO_TDAYS, rolling=USE_ROLLING_WINDOW,
                                       max_train=TRAIN_TDAYS)
    print(f"  {_TRAIN_DIR}: {len(_dir_folds)} folds (rolling={USE_ROLLING_WINDOW})")

    for fold_num, (tr_idx, te_idx) in enumerate(_dir_folds, 1):
        print("\n" + "=" * 90)
        print(f"FOLD {fold_num:02d}/{len(_dir_folds)}")
        print("=" * 90)
    ...
    all_preds_df.to_csv(OUTPUT_DIR / f'wf_results_re28_{_TRAIN_DIR}.csv',              index=False)
    metrics_df.to_csv(OUTPUT_DIR   / f'wf_fold_metrics_re28_{_TRAIN_DIR}.csv',         index=False)
    decile_df.to_csv(OUTPUT_DIR    / f'decile_analysis_re28_{_TRAIN_DIR}.csv',         index=False)
    stability_df.to_csv(OUTPUT_DIR / f'feature_stability_re28_{_TRAIN_DIR}.csv',       index=False)
    if _L2_ENABLED:
        l2_metrics_df.to_csv(OUTPUT_DIR / f'l2_fold_metrics_re28_{_TRAIN_DIR}.csv',        index=False)
        l2_bucket_metrics_df.to_csv(OUTPUT_DIR / f'l2_bucket_metrics_re28_{_TRAIN_DIR}.csv', index=False)
    if not l2_preds_df.empty:
        l2_preds_df.to_csv(OUTPUT_DIR / f'l2_results_re28_{_TRAIN_DIR}.csv',           index=False)
    if oof_moe_records:
        oof_meta_df.to_csv(OUTPUT_DIR / f'oof_moe_records_re28_{_TRAIN_DIR}.csv',      index=False)
    if 'ablation_df' in dir() and not ablation_df.empty:
        ablation_df.to_csv(OUTPUT_DIR / f'ablation_group_importance_re28_{_TRAIN_DIR}.csv', index=False)

    print(f"\n✓ Saved (re25):")
    print(f"  {OUTPUT_DIR}/wf_results_re28_{_TRAIN_DIR}.csv         ({len(all_preds_df):,} rows, Layer 1 daily)")
    print(f"  {OUTPUT_DIR}/wf_fold_metrics_re28_{_TRAIN_DIR}.csv    ({len(metrics_df)} folds)")
    print(f"  {OUTPUT_DIR}/decile_analysis_re28_{_TRAIN_DIR}.csv")
    print(f"  {OUTPUT_DIR}/feature_stability_re28_{_TRAIN_DIR}.csv  ({len(stability_df)} features)")
    print(f"  {OUTPUT_DIR}/l2_fold_metrics_re28_{_TRAIN_DIR}.csv    ({len(l2_metrics_df)} folds)")
    print(f"  {OUTPUT_DIR}/l2_bucket_metrics_re28_{_TRAIN_DIR}.csv")
    if not l2_preds_df.empty:
        print(f"  {OUTPUT_DIR}/l2_results_re28_{_TRAIN_DIR}.csv        ({len(l2_preds_df):,} rows, Layer 2 intraday)")
    if oof_moe_records:
        print(f"  {OUTPUT_DIR}/oof_moe_records_re28_{_TRAIN_DIR}.csv   ({len(oof_meta_df):,} OOF MoE records, Layer 4)")
    if 'ablation_df' in dir() and not ablation_df.empty:
        print(f"  {OUTPUT_DIR}/ablation_group_importance_re28_{_TRAIN_DIR}.csv  ({len(ablation_df)} groups)")
